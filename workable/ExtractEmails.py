...
class ExtractEmails:
    ...
    def extract_emails(self, url):
        print (6)
        try:
            r = requests.get(url, headers=self.headers, verify=self.verify, timeout=(50))
        except:
            r = requests.get("https://"+url, headers=self.headers, verify=self.verify, timeout=(50))
        print (7)
    ...
    def get_emails(self, page):
        ...
        emails = re.findall(r"[a-zA-Z0-9\.\-+_]+@[a-zA-Z0-9\.\-+_]+\.[a-zA-Z]+", page)
        print (25)
...
if __name__=='__main__':
    ...
    count=4
    w=open("bl-emails.txt", 'r')
    emails=[i.strip() for i in w]
    w.close()
    w=open("bl-emails.txt", 'a')

    for i in df['URL'][count:]:
        try:
            em = ExtractEmails(i, print_log=True, user_agent='Mozilla/5.0 (X11; Ubuntu; Linux x86_64; rv:66.0) Gecko/20100101 Firefox/66.0',
                    depth=10)
        except:
            count+=1
            continue
        ...
        for email in em.emails:
            ...
            if verify_email(email)['status'] in ['deliverable', 'risky']:
                print(' either email is deliverable or risky')

    
                listId='6803362'
                print ("adding to backlinks list")

                try:
                    add_prospect_to_list_with_custom(email=email,backlink_site=i,listId=listId)
                    print ('added')
                except:
            
                    print ('error, could not add')
            else:
                print ('not added',verify_email(email)['status'])
            emails.append(email)
            w.write(email+'\n')
            w.close()
            w=open("bl-emails.txt", 'a')
